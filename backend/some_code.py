...
def generate_visualizations(df: pd.DataFrame) -> Dict[str, str]:
    ...
    if len(numeric_cols) > 0:
        ...
        buf.seek(0)
        visualizations["correlation_heatmap"] = base64.b64encode(buf.read()).decode(
            "utf-8"
        )
        plt.close()
        ...
        for i, col in enumerate(numeric_cols[:5]):
            ...
            plt.savefig(buf, format="png")
            buf.seek(0)
            visualizations[f"distribution_{col}"] = base64.b64encode(buf.read()).decode(
                "utf-8"
            )
            plt.close()
    ...
    for i, col in enumerate(categorical_cols[:5]):
        ...
        plt.savefig(buf, format="png")
        buf.seek(0)
        visualizations[f"categorical_{col}"] = base64.b64encode(buf.read()).decode(
            "utf-8"
        )
        plt.close()
...
def generate_insights(df: pd.DataFrame) -> List[str]:
    ...
    if missing.sum() > 0:
        ...
        insights.append(
            f"Found {missing.sum()} missing values across {len(cols_with_missing)} columns."
        )
    ...
    if len(numeric_cols) > 0:
        ...
        if high_corr:
            insights.append(
                f"Found {len(high_corr)} pairs of highly correlated features (>0.8)."
            )
        ...
        for col in numeric_cols:
            ...
            if abs(skew) > 1:
                insights.append(
                    f"Column '{col}' is {'positively' if skew > 0 else 'negatively'} skewed ({skew:.2f})."
                )
    ...
    if len(categorical_cols) > 0:
        for col in categorical_cols:
            ...
            if unique_count == total_count:
                insights.append(
                    f"Column '{col}' has all unique values, might be an ID column."
                )
            elif unique_count == 1:
                insights.append(
                    f"Column '{col}' has only one value: '{df[col].iloc[0]}'."
                )
    ...
    if X_scaled.shape[1] > 1:
        ...
        results["pca"] = {
            ...
            "components": (
                ...
                else "Too many features to display"
            ),
        }
